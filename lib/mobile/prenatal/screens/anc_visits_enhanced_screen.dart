...
class ANCVisitsEnhancedScreen extends StatefulWidget {
  final VoidCallback? onOpenDrawer;
  const ANCVisitsEnhancedScreen({super.key, this.onOpenDrawer});

  @override
  State<ANCVisitsEnhancedScreen> createState() => _ANCVisitsEnhancedScreenState();
}

class _ANCVisitsEnhancedScreenState extends State<ANCVisitsEnhancedScreen> {
  bool _loading = true;
  PregnancyData? _data;
  List<ANCVisitRecord> _visits = [];
  late ANCVisitTracker _tracker;
  ...
  Future<void> _loadData() async {
    try {
      ...
      if (user != null && user.lmpDate.isNotEmpty) {
        ...
        _data = PregnancyData.fromTotalWeeks(20);
      }

      // Load visits from API
      try {
        final data = await ApiService.instance.get('/appointments/anc-visits');
        if (data is List) {
          _visits = (data as List).map((v) => ANCVisitRecord.fromJson(v as Map<String, dynamic>)).toList();
        }
      } catch (_) {
      ...
      _tracker = ANCVisitTracker(visits: _visits);

      if (mounted) {
        setState(() => _loading = false);
      }
    } catch (e) {
      print('Error loading ANC data: $e');
      if (mounted) {
        setState(() => _loading = false);
      }
    }
  ...
  List<ANCVisitRecord> _createDefaultSchedule() {
    if (_data == null) return [];

    final schedule = ANCVisitSchedule.getWHOSchedule();
    return schedule.map((s) {
      final scheduledDate = _data!.lmp.add(Duration(days: s.targetWeek * 7));
      return ANCVisitRecord(
        id: 'anc_${s.visitNumber}',
  ...
  String _getDefaultStatus(DateTime scheduledDate) {
    ...
    if (now.isBefore(scheduledDate.subtract(const Duration(days: 7)))) {
      return 'scheduled';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('ANC Visit Schedule', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w600)),
        leading: IconButton(
                  ...
                  children: [
                    ...
                    ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final schedule = ANCVisitSchedule.getWHOSchedule()[index];
                        return _VisitCard(
                          visit: visit,
                          schedule: schedule,
  ...
  void _showVisitDetails(BuildContext context, ANCVisitRecord visit, ANCVisitSchedule schedule) {
...
class _VisitCard extends StatelessWidget {
  final ANCVisitRecord visit;
  final ANCVisitSchedule schedule;
              ...
              child: Row(
                ...
                children: [
                  Column(
                    ...
                    children: [
                      Text(
                        'ANC Visit ${schedule.visitNumber}',
                        style: const TextStyle(
                          fontSize: 18,
                          fontWeight: FontWeight.bold,
                          color: Colors.white,
