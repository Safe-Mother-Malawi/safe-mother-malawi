...
/// IVR WebSocket Service - Manages real-time alerts for clinician dashboard
class IvrWebSocketService extends ChangeNotifier {
  ...
  Future<void> connect({
    ...
    try {
      ...
      _socket.onConnect((_) {
        ...
        _joinAlerts();
        notifyListeners();
        print('✅ Connected to IVR alerts WebSocket');
      });

      _socket.onDisconnect((_) {
        _isConnected = false;
        notifyListeners();
        print('❌ Disconnected from IVR alerts WebSocket');
      });
      ...
      _socket.on('ivr-alert', (data) {
        print('📨 Received IVR alert: $data');
        final alert = IvrAlert.fromJson(data is Map ? data : jsonDecode(data));
      ...
      _socket.on('joined', (data) {
        print('✅ Joined alerts channel: $data');
      });
