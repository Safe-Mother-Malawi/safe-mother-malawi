...
class ScheduleScreen extends StatefulWidget {
  ...
  const ScheduleScreen({super.key, this.onOpenDrawer});
...
class _CalendarHeader extends StatelessWidget {
  ...
  final VoidCallback? onOpenDrawer;
  const _CalendarHeader({required this.focusedMonth, required this.onPrev,
      required this.onNext, required this.onNew, this.onOpenDrawer});
    ...
    return Container(
      ...
      child: SafeArea(
        ...
        child: Padding(
          ...
          child: Row(children: [
            GestureDetector(onTap: () => onOpenDrawer?.call(),
                child: const Icon(Icons.menu, color: Colors.white, size: 24)),
            const SizedBox(width: 14),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('${_months[focusedMonth.month - 1]} ${focusedMonth.year}',
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(color: const Color(0xFF3949AB), borderRadius: BorderRadius.circular(10)),
                child: const Text('+ New', style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w600)),
              ),
            ),
          ]),
        ),
...
class _CalendarGrid extends StatelessWidget {
  ...
  const _CalendarGrid({required this.focusedMonth, required this.selectedDay,
      required this.appointments, required this.onDayTap});
      ...
      child: Column(children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceAround,
            children: ['S','M','T','W','T','F','S'].map((d) => SizedBox(
              width: 36,
              child: Center(child: Text(d, style: const TextStyle(fontSize: 12,
                  fontWeight: FontWeight.w600, color: Color(0xFF9E9E9E)))),
            )).toList()),
        const SizedBox(height: 8),
        GridView.builder(
          shrinkWrap: true,
          ...
          itemBuilder: (_, i) {
            ...
            return GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: isSelected ? const Color(0xFF1A237E) : isToday ? const Color(0xFFE8EAF6) : Colors.transparent,
                ...
                child: Stack(alignment: Alignment.center, children: [
                  Text('${day.day}', style: TextStyle(
                    ...
                    color: isSelected ? Colors.white : isToday ? const Color(0xFF1A237E) : const Color(0xFF424242),
                  )),
...
class _AppointmentCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final provider = (a['doctor'] ?? a['clinician']?['fullName'] ?? '—').toString();
    final status = (a['status'] ?? 'Pending').toString();

    final diff = date.difference(DateTime.now()).inDays;
    String badgeLabel;
    Color badgeColor, badgeBg;
    if (diff == 0) { badgeLabel = 'Today'; badgeColor = const Color(0xFFC0392B); badgeBg = const Color(0xFFFDECEC); }
    else if (diff == 1) { badgeLabel = 'Tomorrow'; badgeColor = const Color(0xFFC0392B); badgeBg = const Color(0xFFFDECEC); }
    else if (status.toLowerCase() == 'confirmed') { badgeLabel = 'Confirmed'; badgeColor = const Color(0xFF2E7D32); badgeBg = const Color(0xFFE8F5E9); }
    else { badgeLabel = status; badgeColor = const Color(0xFFE65100); badgeBg = const Color(0xFFFFF3E0); }
    ...
    return Container(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          child: Column(children: [
            Text(_months[date.month - 1], style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: Color(0xFF1A237E), letterSpacing: 0.5)),
            Text('${date.day}', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Color(0xFF1A237E), height: 1.1)),
          ]),
        ),
        const SizedBox(width: 14),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Expanded(child: Text(title, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Color(0xFF212121)))),
            const SizedBox(width: 8),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
              decoration: BoxDecoration(color: badgeBg, borderRadius: BorderRadius.circular(20)),
              child: Text(badgeLabel, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: badgeColor)),
            ),
          ]),
          const SizedBox(height: 4),
          Text('$time · $location', style: const TextStyle(fontSize: 12, color: Color(0xFF757575))),
          const SizedBox(height: 2),
          Text(provider, style: const TextStyle(fontSize: 12, color: Color(0xFF757575))),
          const SizedBox(height: 8),
          // Action buttons
          Row(
            children: [
  ...
  Widget build(BuildContext context) {
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: Color(0xFF1A237E))),
      ),
    );
  }
}
  ...
  Future<void> _loadCliniciansAndPatient() async {
    ...
    try {
      ...
      if (user != null && user.facilityName.isNotEmpty) {
        ...
        // Fetch patient ID
        final patientData = await ApiService.instance.get('/patients/me/neonatal');
